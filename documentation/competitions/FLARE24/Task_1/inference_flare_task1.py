...
def predict_flare(input_dir, output_dir, model_folder, folds=("all",)):
    ...
    for input_file, output_file in zip(input_files, output_files):
        ...
        with torch.no_grad():
            ...
            data, _ = preprocessor.run_case_npy(image, None, props, plans_manager, configuration_manager, dataset_json)
            data = torch.from_numpy(data).to(dtype=torch.float32, memory_format=torch.contiguous_format)
            predicted_logits = predictor.predict_logits_from_preprocessed_data(data).cpu()
            export_prediction_from_logits(
                predicted_logits, props, configuration_manager, plans_manager, dataset_json, output_file, False
            )
