...
class CustomPredictor(nnUNetPredictor):
    def predict_single_npy_array(
        self, input_image: np.ndarray, image_properties: dict, segmentation_previous_stage: np.ndarray = None
    ):
        ...
        with torch.no_grad():
            ...
            data, _, image_properties = preprocessor.run_case_npy(
                input_image, None, image_properties, self.plans_manager, self.configuration_manager, self.dataset_json
            )
    ...
    def predict_preprocessed_image(self, image):
        ...
        for p in self.list_of_parameters:
            ...
            self.network.load_state_dict(
                torch.load(p, map_location=compute_device)["network_weights"], weights_only=False
            )
    ...
    def convert_predicted_logits_to_segmentation_with_correct_shape(self, predicted_logits, props):
        ...
        current_spacing = (
            ...
            if len(self.configuration_manager.spacing) == len(props["shape_after_cropping_and_before_resampling"])
...
def predict_semseg(im, prop, semseg_trained_model, semseg_folds):
    ...
    pred_semseg = CustomPredictor(
        tile_step_size=0.5, use_mirroring=True, use_gaussian=True, perform_everything_on_device=False, allow_tqdm=True
    )
    pred_semseg.initialize_from_trained_model_folder(
        semseg_trained_model, use_folds=semseg_folds, checkpoint_name="checkpoint_final.pth"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-sem_mod", "--semseg_trained_model", type=str, default="/opt/app/_trained_model/semseg_trained_model"
    )
