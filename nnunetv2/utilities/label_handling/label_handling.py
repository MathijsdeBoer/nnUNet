...
class LabelManager(object):
    ...
    def _determine_ignore_label(self) -> Union[None, int]:
        ...
        if ignore_label is not None:
            assert isinstance(ignore_label, int), (
                f"Ignore label has to be an integer. It cannot be a region (list/tuple). Got {type(ignore_label)}."
            )
        return ignore_label
    ...
    def apply_inference_nonlin(self, logits: Union[np.ndarray, torch.Tensor]) -> Union[np.ndarray, torch.Tensor]:
        """
    ...
    def convert_probabilities_to_segmentation(
        ...
        if not isinstance(predicted_probabilities, (np.ndarray, torch.Tensor)):
            raise RuntimeError(
                f"Unexpected input type. Expected np.ndarray or torch.Tensor, got {type(predicted_probabilities)}"
            )
        ...
        if self.has_regions:
            if isinstance(predicted_probabilities, np.ndarray):
                ...
                segmentation = torch.zeros(
                    predicted_probabilities.shape[1:], dtype=torch.int16, device=predicted_probabilities.device
                )
    ...
    def revert_cropping_on_probabilities(
        ...
        probs_reverted_cropping = (
            ...
            if isinstance(predicted_probabilities, np.ndarray)
            else torch.zeros((predicted_probabilities.shape[0], *original_shape), dtype=predicted_probabilities.dtype)
        )
...
def determine_num_input_channels(
    plans_manager: PlansManager, configuration_or_config_manager: Union[str, ConfigurationManager], dataset_json: dict
) -> int:
    ...
    num_modalities = (
        len(dataset_json["modality"]) if "modality" in dataset_json.keys() else len(dataset_json["channel_names"])
    )
