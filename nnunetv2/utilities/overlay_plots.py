...
def plot_overlay_preprocessed(
    dataset: nnUNetBaseDataset, k: str, output_folder: str, overlay_intensity: float = 0.6, channel_idx=0
):
    import matplotlib.pyplot as plt
...
def multiprocessing_plot_overlay_preprocessed(
    dataset: nnUNetBaseDataset, output_folder, overlay_intensity, num_processes=8, channel_idx=0
):
    with multiprocessing.get_context("spawn").Pool(num_processes) as p:
        r = []
        for k in dataset.identifiers:
            r.append(
                p.starmap_async(
                    plot_overlay_preprocessed, ((dataset, k, output_folder, overlay_intensity, channel_idx),)
                )
            )
        _ = [i.get() for i in r]
...
def entry_point_generate_overlay():
    ...
    parser.add_argument(
        "--use_raw", action="store_true", required=False, help="if set then we use raw data. else we use preprocessed"
    )
    parser.add_argument(
        "-p",
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="configuration name. Only used if --use_raw is not set! Default: None = 3d_fullres if available, else 2d",
    )
    parser.add_argument(
        "-overlay_intensity",
        type=float,
