...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            ret = [None] * len(r)
            with tqdm(desc="Collecting results", total=len(r), disable=not self.allow_tqdm) as pbar:
    ...
    def predict_from_files_sequential(
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            data, seg, data_properties = preprocessor.run_case(
                li, sps, self.plans_manager, self.configuration_manager, self.dataset_json
            )
            if folder_with_segs_from_prev_stage is not None:
