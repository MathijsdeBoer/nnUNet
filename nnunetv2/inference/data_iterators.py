...
def preprocessing_iterator_fromfiles(
    ...
    while (not done_events[worker_ctr].is_set()) or (not target_queues[worker_ctr].empty()):
        ...
        if not target_queues[worker_ctr].empty():
            ...
            all_ok = (
                all([i.is_alive() or j.is_set() for i, j in zip(processes, done_events)]) and not abort_event.is_set()
            )
            if not all_ok:
                raise RuntimeError(
...
class PreprocessAdapter(DataLoader):
    def __init__(
        ...
        super().__init__(
            list(zip(list_of_lists, list_of_segs_from_prev_stage_files, output_filenames_truncated)),
            1,
    ...
    try:
        ...
        for idx in range(len(list_of_images)):
            data, seg, props = preprocessor.run_case_npy(
                list_of_images[idx],
                list_of_segs_from_prev_stage[idx] if list_of_segs_from_prev_stage is not None else None,
