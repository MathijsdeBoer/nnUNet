...
def convert_autopet(
    autopet_base_dir: str = "/media/isensee/My Book1/AutoPET/nifti/FDG-PET-CT-Lesions", nnunet_dataset_id: int = 221
):
    ...
    for pat in patients:
        ...
        for pa in patient_acquisitions:
            ...
            if not isfile(join(imagestr, f"{identifier}_0000.nii.gz")):
                shutil.copy(
                    join(autopet_base_dir, pat, pa, "CTres.nii.gz"), join(imagestr, f"{identifier}_0000.nii.gz")
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input_folder", type=str, help="The downloaded and extracted autopet dataset (must have PETCT_XXX subfolders)"
    )
