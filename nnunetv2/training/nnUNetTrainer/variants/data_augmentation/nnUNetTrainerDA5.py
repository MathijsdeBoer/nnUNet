    ...
    def get_training_transforms(
        ...
        if np.any(matching_axes > 1):
            transforms.append(
                RandomTransform(
                    Rot90Transform(
                        num_axis_combinations=1, num_rot_per_combination=(0, 1, 2, 3), allowed_axes=set(valid_axes)
                    ),
                ...
                [
                    ...
                    RandomTransform(
                        GaussianBlurTransform(
                            (0.3, 1.5), synchronize_channels=False, synchronize_axes=True, p_per_channel=0.5
                        ),
                        apply_probability=0.2,
                    ),
                ]
        ...
        transforms.append(
            RandomTransform(
                SharpeningTransform(strength=(0.1, 1), p_same_for_each_channel=0, p_per_channel=0.5),
                apply_probability=0.2,
            )
        )
