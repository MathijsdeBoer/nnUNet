...
from nnunetv2.training.nnUNetTrainer.variants.cb_dice.torch_nn import BasicConv, batched_index_select, act_layer
from nnunetv2.training.nnUNetTrainer.variants.cb_dice.torch_edge import DenseDilatedKnnGraph
from nnunetv2.training.nnUNetTrainer.variants.cb_dice.pos_embed import (
...
class OptInit:
    def __init__(self, drop_path_rate=0.0, pool_op_kernel_sizes_len=4):
                ...
                stage_modules.append(
                    nn.Sequential(
                        ...
                        PoolGNN_blocks(
                            features_per_stage[s],
                            img_shape_list[s],
                            s - self.no_pool_gnn_stage_num,
                            self.no_pool_gnn_stage_num,
                            opt=self.opt,
                            conv_op=conv_op,
                            norm_op=norm_op,
                            norm_op_kwargs=norm_op_kwargs,
...
class NexToU_Decoder(nn.Module):
    def __init__(
        ...
        if encoder.conv_op == nn.Conv2d:
            ...
            raise ValueError("unknown convolution dimensionality, conv op: %s" % str(encoder.conv_op))
            ...
            if s < (n_stages_encoder - self.no_pool_gnn_stage_num):
                stages.append(
                    nn.Sequential(
                        ...
                        PoolGNN_blocks(
                            input_features_skip,
                            img_shape_list[n_stages_encoder - (s + 1)],
                        ...
                        Swin_GNN_blocks(
                            input_features_skip,
                            img_shape_list[n_stages_encoder - (s + 1)],
                            n_stages_encoder - self.n_conv_stages - (s + 1),
                            opt=self.opt,
                            conv_op=encoder.conv_op,
                            norm_op=encoder.norm_op,
                            norm_op_kwargs=encoder.norm_op_kwargs,
...
class MRConv(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, act="relu", norm=None, bias=True, conv_op=nn.Conv3d, dropout_op=nn.Dropout3d
    ):
...
class Grapher(nn.Module):
    ...
    def __init__(
        ...
        if relative_pos:
            if self.conv_op == nn.Conv2d:
                relative_pos_tensor = (
                    torch.from_numpy(np.float32(get_2d_relative_pos_embed(in_channels, int(n ** (1 / 2)))))
                    .unsqueeze(0)
                    .unsqueeze(1)
                )
                ...
                relative_pos_tensor = (
                    torch.from_numpy(np.float32(get_3d_relative_pos_embed(in_channels, int(n ** (1 / 3)))))
                    .unsqueeze(0)
                    .unsqueeze(1)
                )
                relative_pos_tensor = F.interpolate(
...
def window_partition(x, window_size):
    ...
    if len(x.shape) == 4:
        B, C, H, W = x.shape
        x = x.permute(0, 2, 3, 1)
        windows = rearrange(x, "b (h p1) (w p2) c -> (b h w) p1 p2 c", p1=window_size[0], p2=window_size[1], c=C)
        ...
        windows = rearrange(
            x,
            "b (s p1) (h p2) (w p3) c -> (b s h w) p1 p2 p3 c",
            p1=window_size[0],
            p2=window_size[1],
            p3=window_size[2],
            c=C,
...
def window_reverse(windows, window_size, size_tuple):
    ...
    if len(windows.shape) == 4:
        ...
        x = rearrange(
            windows,
            "(b s h w) p1 p2 p3 c -> b (s p1) (h p2) (w p3) c",
            p1=window_size[0],
            p2=window_size[1],
            p3=window_size[2],
            b=B,
    ...
    def forward(self, x):
        ...
        if max(self.shift_size) > 0 and self.conv_op == nn.Conv2d:
            ...
            shifted_x = torch.roll(
                x, shifts=(-self.shift_size[0], -self.shift_size[1], -self.shift_size[2]), dims=(2, 3, 4)
            )
        else:
            shifted_x = x
        ...
        if max(self.shift_size) > 0 and self.conv_op == nn.Conv2d:
            ...
            x = torch.roll(
                shifted_x, shifts=(self.shift_size[0], self.shift_size[1], self.shift_size[2]), dims=(2, 3, 4)
            )
        else:
            x = shifted_x
...
class PoolGrapher(nn.Module):
    ...
    def __init__(
        ...
        self.graph_conv = PoolDyGraphConv(
            in_channels,
            in_channels * 2,
            kernel_size,
            dilation,
        ...
        **kwargs,
        ...
        if conv_op == nn.Conv2d:
            ...
            if pool_op_kernel_sizes_len >= 5:
                k = [min(4, max_k), min(4, max_k), min(4, max_k), min(8, max_k), min(16, max_k)] + [min(32, max_k)] * (
                    pool_op_kernel_sizes_len - 5
                )
            else:
                k = [min(4, max_k), min(4, max_k), min(4, max_k), min(8, max_k), min(16, max_k)][
                    0:pool_op_kernel_sizes_len
                ]
        ...
        if conv_op == nn.Conv2d:
            ...
            if pool_op_kernel_sizes_len >= 5:
                k_list = [
                    ...
                    min(min_k * 4, max_k),
                    min(min_k * 8, max_k),
                ] + [min(min_k * 16, max_k)] * (pool_op_kernel_sizes_len - 5)
            else:
                k_list = [
                    min(min_k, max_k),
                    min(min_k * 2, max_k),
            ...
            blocks.append(
                nn.Sequential(
                    PoolGrapher(
                        ...
                        stochastic,
                        epsilon,
                        reduce_ratios[i + stage_num],
                        n=n_size_list[i + stage_num],
                        drop_path=dpr[idx],
                        relative_pos=True,
                        conv_op=conv_op,
                        norm_op=norm_op,
