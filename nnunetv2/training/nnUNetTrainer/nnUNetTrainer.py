...
class nnUNetTrainer(object):
    def __init__(
        ...
        self.preprocessed_dataset_folder_base = (
            join(nnUNet_preprocessed, self.plans_manager.dataset_name) if nnUNet_preprocessed.is_set() else None
        )
        ...
        self.output_folder = (
            join(self.output_folder_base, f"fold_{fold}") if self.output_folder_base is not None else None
        )

        self.preprocessed_dataset_folder = (
            join(self.preprocessed_dataset_folder_base, self.configuration_manager.data_identifier)
    ...
    def _set_batch_size_and_oversample(self):
        if not self.is_ddp:
            ...
            # compute oversample
            oversample = [
                True if not i < round(global_batch_size * (1 - self.oversample_foreground_percent)) else False
            ...
            transforms.append(
                RandomTransform(
                    ApplyRandomBinaryOperatorTransform(
                        channel_idx=list(range(-len(foreground_labels), 0)), strel_size=(1, 8), p_per_label=0.5
                    ),
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file(
            "Pseudo dice", [np.round(i, decimals=4) for i in self.logger.get_value("dice_per_class_or_region", step=-1)]
        )
        self.print_to_log_file(
