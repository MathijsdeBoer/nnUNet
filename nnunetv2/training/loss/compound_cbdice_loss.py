...
class DC_and_CE_and_CBDC_loss(nn.Module):
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor, t_skeletonize_flag=False):
        """
        ...
        cbdice_loss = (
            self.cbdice(net_output, target, t_skeletonize_flag=t_skeletonize_flag) if self.weight_cbdice != 0 else 0
        )
...
class DC_and_CE_and_CL_M_DC_loss(nn.Module):
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor, t_skeletonize_flag=False):
        ...
        clMdice_loss = (
            self.clMdice(net_output, target, t_skeletonize_flage=t_skeletonize_flage) if self.weight_clMdice != 0 else 0
        )
