...
class BoundaryDoULoss(nn.Module):
    ...
    def _adaptive_size(self, score, target):
        ...
        padding_out = torch.zeros((target.shape[0], target.shape[-2] + 2, target.shape[-1] + 2), device=target.device)
        padding_out[:, 1:-1, 1:-1] = target
        h, w = 3, 3

        Y = torch.zeros(
            (padding_out.shape[0], padding_out.shape[1] - h + 1, padding_out.shape[2] - w + 1), device=target.device
        )
