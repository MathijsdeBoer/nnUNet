...
class ExperimentPlanner(object):
    def __init__(
        ...
        if isfile(join(self.raw_dataset_folder, "splits_final.json")):
            _maybe_copy_splits_file(
                join(self.raw_dataset_folder, "splits_final.json"), join(preprocessed_folder, "splits_final.json")
            )
    ...
    def static_estimate_VRAM_usage(
        ...
        net = get_network_from_plans(
            arch_class_name, arch_kwargs, arch_kwargs_req_import, input_channels, output_channels, allow_init=False
        )
    ...
    def get_plans_for_configuration(
        ...
        network_num_pool_per_axis, pool_op_kernel_sizes, conv_kernel_sizes, patch_size, shape_must_be_divisible_by = (
